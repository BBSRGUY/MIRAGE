...
from ..compression.streamed_fit import fit_streamed_shared_basis, streamed_pca_basis
...
def run_basis_sweep(config: M2Config, device: str | torch.device | None = None) -> dict[str, Any]:
    ...
    groups: dict[tuple[str, tuple[int, ...]], list[Any]] = defaultdict(list)
    for record in store.records(kind="weight"):
        name = str(record.metadata["name"])
        shape = tuple(int(value) for value in record.metadata["shape"])
        groups[(projection_family(name), shape)].append(record)
    if not groups:
    ...
    for (family, shape), records in sorted(groups.items()):
        records.sort(key=lambda record: str(record.metadata["name"]))
        names = tuple(str(record.metadata["name"]) for record in records)
        layer_count = len(records)
        source_bytes = layer_count * shape[0] * shape[1] * 4
        limit = config.compression.max_fit_vram_gb * 2**30
        use_streamed = device.type == "cuda" and 3 * source_bytes > limit
        streamed_basis = streamed_alpha = streamed_eigenvalues = None
        if use_streamed:
            maximum_basis = max(
                count for count in config.compression.basis_counts if count <= layer_count
            )
            streamed_basis, streamed_alpha, streamed_eigenvalues = streamed_pca_basis(
                store,
                records,
                shape,
                basis_count=maximum_basis,
                device=device,
            ...
            for rank in config.compression.ranks:
                ...
                if use_streamed:
                    result = fit_streamed_shared_basis(
                        ...
                        device=device,
                        row_chunk_size=config.compression.streamed_row_chunk_size,
                        precomputed_basis=streamed_basis,
                        precomputed_alpha=streamed_alpha,
                        eigenvalues=streamed_eigenvalues,
                    )
                else:
                    tensors = [
                        store.load(record, device="cpu")["weight"].float()
                        for record in records
                    ]
                    weights = torch.stack(tensors).to(device)
                    del tensors
